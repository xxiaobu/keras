...
class TestDistributionStrategyDnnCorrectness(tf.test.TestCase,
  ...
  def test_fused_batch_norm_uneven_batch(self, distribution):
    ...
    but using ResNet as an example model using fused batchnorm.
    ...
    # set data / batch size to have one normal batch, then one uneven batch
    global_batch_size = strategy.num_replicas_in_sync
    num_samples = 2 * global_batch_size - 1
